							...
							char** strWhere)
{
	...
	char strpW[SE_MAX_MESSAGE_LENGTH] ={0};
								    ...
								    void* pSpatialFilter,CDistIntArray* pData)
{
	...
	sqlc->where = (char*) malloc(sizeof(char)*SE_MAX_MESSAGE_LENGTH);    
	memset(sqlc->where,0,sizeof(char)*SE_MAX_MESSAGE_LENGTH);
	if(NULL != strWhere) 
...
long CSdeCommand::SelectData(const char* strSQL,IDistRecordSet** IRcdSet,void* pSpatialFilter)
{
	...
	check_error (rc,  "SelectData->SE_queryinfo_set_table");  //设置表名

	rc = SE_queryinfo_set_columns (queryinfo,nFieldNum,(const char **)strFieldNames);
	check_error (rc, "SelectData->SE_queryinfo_set_table");  //设置字段名
...
long CSdeCommand::InsertData(int* pRowId,const char* strSQL,...)
{
	...
	SE_BLOB_INFO  *pBlob = NULL; SE_SHAPE pShape = NULL;DIST_DATETIME_X* pDate=NULL; tm DateTime;
	...
	for(j=0; j<nFieldNum; j++)
	{
		...
		case kBLOB:
			...
			break;
		case kDate:	        
			pDate= va_arg(lpParam,DIST_DATETIME_X*);
			if(pDate != NULL)
			{
				DateTime.tm_sec  = pDate->tm_sec;
				DateTime.tm_min  = pDate->tm_min;
				DateTime.tm_hour = pDate->tm_hour;
				DateTime.tm_mday = pDate->tm_mday;
				DateTime.tm_mon  = pDate->tm_mon;
				DateTime.tm_year = pDate->tm_year;
				DateTime.tm_wday = pDate->tm_wday;
				DateTime.tm_yday = pDate->tm_yday;
				DateTime.tm_isdst = pDate->tm_isdst;
				rc = SE_stream_set_date(pStream, j+1,(const tm*)(&DateTime));
			}
			else
			{
...
long CSdeCommand::InsertDataAsString(int* pRowId,const char* strSQL,char** strValueArray)
{
	...
	//4. 根据字段信息，从开发参数结构中取参数值
	short int sValue=0; int nValue=0; float fValue=0; double dValue=0; char* strValue=NULL;
	SE_BLOB_INFO  *pBlob = NULL; struct tm DateTimes; SE_SHAPE pShape = NULL; DIST_DATETIME_X* pDate = NULL;
	SE_WCHAR strValueW[SE_MAX_MESSAGE_LENGTH];
	...
	for(j=0; j<nFieldNum; j++)
	{
		...
		case kDate:
			...
			{
				nValue = atoi(cValue);	
				pDate = (DIST_DATETIME_X*) nValue;

				DateTimes.tm_sec = pDate->tm_sec;
				DateTimes.tm_min = pDate->tm_min;
				DateTimes.tm_hour = pDate->tm_hour;
				DateTimes.tm_mday = pDate->tm_mday;
				DateTimes.tm_mon = pDate->tm_mon;
				DateTimes.tm_year = pDate->tm_year;
				DateTimes.tm_wday = pDate->tm_wday;
				DateTimes.tm_yday = pDate->tm_yday;
				DateTimes.tm_isdst = pDate->tm_isdst;
				rc = SE_stream_set_date(pStream, j+1,((const tm*)(&DateTimes)));
			}
			else
				rc = SE_stream_set_date(pStream, j+1,NULL);
...
long CSdeCommand::UpdateData(const char* strSQL,void* pSpatialFilter,...)
{
	...
	//4. 根据字段信息，从开发参数结构中取参数值
	short int sValue=0; int nValue=0; float fValue=0; double dValue=0; char* strValue=NULL;
	SE_BLOB_INFO  *pBlob = NULL; tm DateTime; SE_SHAPE pShape = NULL; DIST_DATETIME_X* pDate=NULL;
	SE_WCHAR strValueW[SE_MAX_MESSAGE_LENGTH];
...
long CSdeCommand::GetTableInfo(IDistParameter** pParam,int* nPamramNum,const char* strName,const char* strOwnerGDB)
{
	...
	*nPamramNum = m_nParamCount;
	...
	//根据图层名信息，分解获取它的拥有者
	char sOwnerName[160],sSdeName[160];
	...
	if(strOwnerGDB == NULL)
		strcpy(strTempGDB,m_strUserName); //默认为当前数据库连接对象的登陆用户名
	else
	...
	strcpy(strTables[0],"SDE.GDB_OBJECTCLASSES");//(因为别名有时候取不出来,所以给固定名称)
	...
	check_error (rc, "GetTableInfo->SE_queryinfo_set_table");  //设置字段名 
	...
	long nID = -1;
	char strTempOwner[SE_MAX_OWNER_LEN]={0};
	char strTempName[SE_MAX_TABLE_LEN]={0};
	while(SE_SUCCESS == SE_stream_fetch(pStream))
	{
		memset(strTempOwner,0,sizeof(char)*SE_MAX_OWNER_LEN);
		memset(strTempName,0,sizeof(char)*SE_MAX_TABLE_LEN);
		
		SE_stream_get_string(pStream,2,strTempOwner);  // "OWNER"
		SE_stream_get_string(pStream,3,strTempName);   // "NAME"

		char strTotalName[260]={0};memset(strTotalName,0,sizeof(char)*260);
		sprintf(strTotalName,"%s.%s",strTempOwner,strTempName);
		//if(CompareString(sOwnerName,strTempOwner)==0 && 
		if(CompareString(strName,strTotalName)==0) //比较是否符合条件  (********************)
		{
	...
	strcpy(strTables[0],"SDE.GDB_FIELDINFO");
	...
	char strFName[SE_MAX_COLUMN_LEN]={0};
	char strAFName[SE_MAX_ALIAS_LEN]={0};
	while(SE_SUCCESS == SE_stream_fetch(pStream))
	{
		SE_stream_get_string(pStream,1,strFName);
		SE_stream_get_string(pStream,2,strAFName);
...
void* CSdeRecordSet::operator[] (int nIndex)
{
	...
	case kDate:	
			tm DateTime;
			rc = SE_stream_get_date(m_pStream, nIndex,(tm*)&DateTime);
			if(rc == SE_NULL_VALUE)
			{
				return NULL;
			}
			...
			if(rc ==SE_SUCCESS)
			{
				m_tmValue.tm_sec = DateTime.tm_sec;
				m_tmValue.tm_min = DateTime.tm_min;
				m_tmValue.tm_hour = DateTime.tm_hour;
				m_tmValue.tm_mday = DateTime.tm_mday;
				m_tmValue.tm_mon = DateTime.tm_mon;
				m_tmValue.tm_year = DateTime.tm_year;
				m_tmValue.tm_wday = DateTime.tm_wday;
				m_tmValue.tm_yday = DateTime.tm_yday;
				m_tmValue.tm_isdst = DateTime.tm_isdst;
			}
			
			check_error(rc, "operator[int]->SE_stream_get_date");
			return (void*) &m_tmValue;
	case kShape:
			if(NULL != m_shape)
				SE_shape_free(m_shape);
...
long CSdeRecordSet::GetValueAsString(char *strValue,int nIndex)
{
	...
	case kDate:	
			...
			//m_nValue32 = int(&m_tmValue);
			sprintf(strValue,"%d-%d-%d",DateTime.tm_year+1900,DateTime.tm_mon+1,DateTime.tm_mday);
			//strcpy(strValue,asctime(&DateTime));
			//sprintf(strValue,"%d",m_nValue32);
			if(rc == SE_SUCCESS)
				return 1;
			else
