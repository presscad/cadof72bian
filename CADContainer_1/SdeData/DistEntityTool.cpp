...
static BOOL IsFileExists(const CString strFName)
{
...
long CDistEntityTool::GetTextStyleName(char *strTextStyleName,AcDbObjectId& ObjId,AcDbDatabase *pDB)
{
	...
	for (; !pTsIter->done(); pTsIter->step()) 
	{
		...
		{
			pTsRecord->getName(strTextStyleName);
			pTsRecord->close();
			break;
		}
		pTsRecord->close();
...
AcDbObjectId CDistEntityTool::CreateBlockByDwgFile(CString strBlkName,CString strDwgFileName)//含路径的文件名(块名为文件名，不含路径)
{
	AcDbObjectId ObjId;
	...
	if(IsFileExists(strDwgFileName)==FALSE)
	{
		strDwgFileName.Format("%sBLOCKS\\BlockName.dwg",GetCurArxAppPath("shSdeData.arx"));	
	}
...
long CDistEntityTool::CreateBlkRefAttr(AcDbObjectId& resultId,AcGePoint3d basePt,char* strBlkName,
									   char* strLyName,int  nClrIndex,char *strScale,double dAngle,
									   CString strAttValue,AcDbDatabase* pDB)
{
	resultId.setNull();

	// 获得当前数据库的块表
	AcDbBlockTable *pBlkTbl=NULL;
	if(Acad::eOk != pDB->getBlockTable(pBlkTbl, AcDb::kForWrite)) return 0;

	// 查找用户指定的块定义是否存在
	AcDbObjectId blkDefId;
	if(Acad::eOk!=pBlkTbl->getAt(strBlkName,blkDefId,AcDb::kForRead)) 
	{
		pBlkTbl->close(); return 0;
	}
	...
	if (pBlkDefRcd->hasAttributeDefinitions())
	{
		...
		for (pItr->start(); !pItr->done(); pItr->step())
		{
			...
			if (pAttDef != NULL)
			{
				...
				pAtt->setTextString(strAttValue);
