...
{
	XmlDocPtr = NULL;	
	//xml_file_name = g_GlobeInfo.m_sSettingPath;
}
...
void CXMLFile::clear()
{
	...
	load((CString)(xml_file_name.c_str()));
}

//---------------------------------------------------------------------------
int CXMLFile::GetData(LPCTSTR lpAppName, 
					   LPCTSTR lpKeyName, 
					   int nDefault)
{	
	int nReturn = nDefault;
	GetData(lpAppName,lpKeyName,nDefault,nReturn);					   
	return nReturn;
}

double CXMLFile::GetData(LPCTSTR lpAppName, 
						  LPCTSTR lpKeyName, 
						  double dDefault)
{	
	double dReturn = dDefault;
	GetData(lpAppName, lpKeyName, dDefault, dReturn);
	return dReturn;
}

CString CXMLFile::GetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, CString strDefault)
{
	CString strReturne = strDefault;
	GetData(lpAppName,lpKeyName,LPCTSTR(strDefault),strReturne);
	return strReturne;
} 


BOOL CXMLFile::GetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, int nDefault, int &nReturn)
{
	nReturn = GetLong(lpAppName, lpKeyName, nDefault);
	return TRUE;
}

BOOL CXMLFile::GetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, double dDefault, double &dReturn)
{
	CString sTemp = _T("");
	sTemp.Format("%.6f", dDefault);	
	dReturn = atof(GetString(lpAppName, lpKeyName, sTemp).c_str());
	return TRUE;
}

BOOL CXMLFile::GetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, CString strDefault, CString& strReturne)
{
	strReturne = GetString(lpAppName, lpKeyName, strDefault).c_str();
	return TRUE;
}
//---------------------------------------------------------------------------	
BOOL CXMLFile::SetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, int nData)
{
	SetLong(lpAppName, lpKeyName, nData);
	return TRUE;
}

BOOL CXMLFile::SetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, LPCTSTR lpString)
{	
	SetString(lpAppName, lpKeyName, lpString);
	return TRUE;
}

BOOL CXMLFile::SetData(LPCTSTR lpAppName, LPCTSTR lpKeyName, double dData)
{
	CString sTemp = _T("");
	sTemp.Format("%.6f", dData);	
	SetString(lpAppName, lpKeyName, sTemp);
	return TRUE;
}
//---------------------------------------------------------------------------
...
long CXMLFile::SetLong(const char* cstrBaseKeyName, const char* cstrValueName, long lValue)
{
	long lRetVal = 0;
	CString sValue = _T("0");
	sValue.Format(_T("%d"), lValue);

	lRetVal = SetString(cstrBaseKeyName, cstrValueName, sValue);
...
long CXMLFile::GetKeysValue(CString i_sBaseKey, CStringArray &o_arsChildKeys)
{
	std::vector<std::string> keys;
	GetKeys(i_sBaseKey, keys);
	
	for( int i = 0; i < keys.size(); i++ )
	{
		o_arsChildKeys.Add((CString)(keys[i].c_str()));
	}

	return o_arsChildKeys.GetSize();
}
