...
public:
	...
	long CreateText(AcDbObjectId& resultId,AcGePoint3d& ptAlign,char* strText,char* strLyName="0",int  nClrIndex=0,char* strFontType="Standard",
		            int nHorzAlignType=0,int nVertAlignType=0,double dHeight=3.0,double dWidFactor=0.8,double dAngle=0.0, AcDbDatabase* pDB = CURDBPOINTER);
...
public:
	...
	void OpenNewDwgFile(CString strFileName);
	...
	BOOL InsertBlkAttrRef(AcDbObjectId& resultId,char* strBlkName,char* strCadLyName,AcGePoint3d pt,
	...
	AcDbObjectId CreateBlockByDwgFile(CString strBlkName,CString strSubPath=""); // **\**

	AcDbViewTableRecord GetCurrentView();

	long CreatePolyline_(AcDbObjectId& resultId,AcGePoint3dArray& ptArray,AcGeDoubleArray&  BlgArray,char* strLyName,
		                 int nClrIndex,bool bIsClosed,double dGlobeWeigth,AcDbDatabase* pDB= CURDBPOINTER);

	long GetPolylinePts(AcDbPolyline* pline,AcGePoint3dArray& ptArray,AcGeDoubleArray& BlgArray);

private:
	static void OpenDocHelper(void *pData);
	static void OpenNewDocHelper(void *pData);
};
