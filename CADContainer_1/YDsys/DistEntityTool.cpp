										  ...
										  
{
	...
	if(Acad::eOk != pDB->getSymbolTable(pLyTable, AcDb::kForRead)) return 0;
	...
	try
	{
		...
		for(int i=0; i<nLen; i++)
		{
			if(strScale[i] == '#' && nPos > 0)
			{
				...
				nPos = 0;  nIndex++; memset(cValue,0,sizeof(char)*20);
			}
...
long  CDistEntityTool::GetPolylinePts(AcDbPolyline *pPolyline,AcGePoint3dArray& ptArray)
{
	...
	const double dPercent = 0.05;

	//获取点数量
	int nNumVert = pPolyline->numVerts(); 

	//读点坐标和其凸度
	for(int i=0;i<nNumVert;i++)  
	{
		pPolyline->getPointAt(i,ptAt);   VertPtArray.append(ptAt);
		pPolyline->getBulgeAt(i,dBulge); VertBlgArray.append(dBulge);
	}

	//闭合,再加首点
	if(pPolyline->isClosed())
	{
		VertPtArray.append(VertPtArray[0]); VertBlgArray.append(0);
	}
	int nPtNum = VertPtArray.logicalLength();
	if(nPtNum < 2) return 0;
	...
	for(i=0;i<nPtNum-1;i++)
	{
		...
		{ 
			nTempLen = ptArray.length(); //保证去除相邻重复点
			if(nTempLen>0)
			{
				ptLast = ptArray.at(nTempLen-1);
				if((fabs(ptLast.x-VertPtArray[i].x)<dPercent)&&(fabs(ptLast.y-VertPtArray[i].y)<dPercent))
				{
					//ptLast.x = VertPtArray[i].x; ptLast.y = VertPtArray[i].y; ptLast.z = 0;
					//ptArray.setAt(nTempLen-1,ptLast); continue;
					continue;
				}
			}
		...
		else
		{
			...
			int nPart = dArcLen * 4; //2.0;
			if(nPart <10) nPart = 10;

			tempPtArray.setLogicalLength(0); 
			ReduceBulge(tempPtArray,VertPtArray[i],VertPtArray[i+1],VertBlgArray[i],nPart);
			for(int j=0;j<tempPtArray.logicalLength();j++)
			{
				nTempLen = ptArray.length(); //保证去除相邻重复点
				if(nTempLen>0)
				{
					ptLast = ptArray.at(nTempLen-1);
					if((fabs(ptLast.x-tempPtArray[j].x)<dPercent)&&(fabs(ptLast.y-tempPtArray[j].y)<dPercent)) continue;
				}
				ptArray.append(AcGePoint3d(tempPtArray[j].x,tempPtArray[j].y,0));
			}
		}

	}

	nTempLen = ptArray.length();
	if(nTempLen>0)  //保证去除相邻重复点
	{
		ptLast = ptArray.at(nTempLen-1);
		if((fabs(ptLast.x-VertPtArray[nPtNum-1].x)>dPercent) || (fabs(ptLast.y-VertPtArray[nPtNum-1].y)>dPercent))
		{
			ptArray.append(AcGePoint3d(VertPtArray[nPtNum-1].x,VertPtArray[nPtNum-1].y,0));
		}
	}
...
void CDistEntityTool::OpenNewDocHelper(void *pData)
{
	if(acDocManager->isApplicationContext())
	{
		CString* dwg = (CString*)pData;
		Acad::ErrorStatus result = acDocManager->appContextNewDocument(*dwg);
		if(result != Acad::eOk)
			acutPrintf("\nERROR: %s\n", acadErrorStatusText(result));
	}
}

void CDistEntityTool::OpenNewDwgFile(CString strFileName)
{
	if(acDocManager->isApplicationContext())
	{
		Acad::ErrorStatus result = acDocManager->appContextNewDocument(strFileName);
		if(result != Acad::eOk)
			acutPrintf("\nERROR: %s\n", acadErrorStatusText(result));
	}
	else
...
BOOL CDistEntityTool::InsertBlkAttrRef(AcDbObjectId& resultId,char* strBlkName,char* strCadLyName,AcGePoint3d pt,
									   double dFontHeight,CStringArray& strTagArray,CStringArray& strTextArray)
{
	// 查找用户指定的块定义是否存在
	AcDbObjectId blkDefId;	
	blkDefId.setNull();
	blkDefId = CreateBlockByDwgFile(strBlkName);
	if(blkDefId.isNull()) return FALSE;
	...
	AcDbObjectId lyId;
	tempTool.CreateLayer(lyId,strCadLyName);
	...
	pBlkTblRcd->appendAcDbEntity(resultId, pBlkRef);  // 将块参照添加到模型空间

	// 判断指定的块表记录是否包含属性定义
	AcDbBlockTableRecord *pBlkDefRcd = NULL;
	if(Acad::eOk != acdbOpenObject(pBlkDefRcd, blkDefId, AcDb::kForRead))
	{
		pBlkTblRcd->close();
...
AcDbObjectId CDistEntityTool::CreateBlockByDwgFile(CString strBlkName,CString strSubPath)//块名(文件名由块名合成)
{
	CString strDwgFileName;
	if(strSubPath.IsEmpty())
		strDwgFileName.Format("%sBLOCKS\\%s.dwg",GetCurArxAppPath("shYDsys.arx"),strBlkName);
	else
		strDwgFileName.Format("%sBLOCKS\\%s\\%s.dwg",GetCurArxAppPath("shYDsys.arx"),strSubPath,strBlkName);
	...
	if((es=acdbCurDwg()->getBlockTable(pTb,AcDb::kForRead))!=Acad::eOk) return ObjId;

	if((es=pTb->getAt(strBlkName,ObjId))==Acad::eOk)
	{
	...
	if((es=pDb->readDwgFile(strDwgFileName))!=Acad::eOk)
	{
		delete pDb; return NULL;
	}
	es = acdbCurDwg()->insert(ObjId,strBlkName,pDb);
									   ...
									   CStringArray& strTagArray,CStringArray& strTextArray,AcDbDatabase* pDB)
{
	...
	// 获得当前数据库的块表
	AcDbBlockTable *pBlkTbl=NULL;
	if((es= pDB->getBlockTable(pBlkTbl, AcDb::kForRead))!=Acad::eOk) return FALSE;
...
long CDistEntityTool::CreatePolyline_(AcDbObjectId& resultId,AcGePoint3dArray& ptArray,AcGeDoubleArray&  BlgArray,char* strLyName,
									  int nClrIndex,bool bIsClosed,double dGlobeWeigth,AcDbDatabase* pDB)
{
	resultId.setNull();
	int nCount = ptArray.length();
	AcDbPolyline* pNewPline = new AcDbPolyline(nCount);    // optimized to know how many verts
	...
	for (int i=0;i<nCount;i++) 
	{
		pt.set(ptArray[i].x, ptArray[i].y);   
		double dd = BlgArray.at(i);
		pNewPline->addVertexAt(i, pt,dd,dGlobeWeigth,dGlobeWeigth);//设置全局宽度
	}

	if(bIsClosed)
		pNewPline->setClosed(Adesk::kTrue);
...
long CDistEntityTool::GetPolylinePts(AcDbPolyline* pline,AcGePoint3dArray& ptArray,AcGeDoubleArray& BlgArray)
{
	ptArray.setLogicalLength(0);
	BlgArray.setLogicalLength(0);
	...
	for(int i=0;i<nNumVert;i++)           //读点坐标和其凸度
	{
		pline->getPointAt(i,tempPt);
		pline->getBulgeAt(i,dBulge);
		ptArray.append(tempPt);
		BlgArray.append(dBulge);
	}
