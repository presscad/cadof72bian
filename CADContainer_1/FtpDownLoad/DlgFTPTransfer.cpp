...
///////////////////////////////// Implementation //////////////////////////////
IMPLEMENT_DYNAMIC(CDlgFTPTransfer, CDialog);

CDlgFTPTransfer::CDlgFTPTransfer(CWnd* pParent /*=NULL*/)
: CDialog(CDlgFTPTransfer::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlgFTPTransfer)
	//}}AFX_DATA_INIT
...
void CDlgFTPTransfer::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgFTPTransfer)
...
BEGIN_MESSAGE_MAP(CDlgFTPTransfer, CDialog)
	//{{AFX_MSG_MAP(CDlgFTPTransfer)
	ON_WM_DESTROY()
	ON_WM_CLOSE()
	//}}AFX_MSG_MAP
...
void CDlgFTPTransfer::AttachToExisitingHandles(HINTERNET hInternetSession, HINTERNET hFTPConnection)
{
...
LRESULT CDlgFTPTransfer::OnAskOverwrite(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	return 1L;
	/*
	CString sMsg;
...
LRESULT CDlgFTPTransfer::OnThreadFinished(WPARAM wParam, LPARAM /*lParam*/)
{
...
BOOL CDlgFTPTransfer::OnInitDialog() 
{
	...
	m_ctrlAnimate.Open(m_bDownload ? IDR_AVI_DOWNLOAD : IDR_AVI_UPLOAD);
...
UINT CDlgFTPTransfer::_TransferThread(LPVOID pParam)
{
	//Convert from the SDK world to the C++ world
	CDlgFTPTransfer* pDlg = (CDlgFTPTransfer*) pParam;
	ASSERT(pDlg);
	ASSERT(pDlg->IsKindOf(RUNTIME_CLASS(CDlgFTPTransfer)));
	pDlg->TransferThread();
	return 0;
}

void CDlgFTPTransfer::SetPercentage(int nPercentage)
{
...
void CDlgFTPTransfer::SetProgressRange(short nLower, short nUpper)
{
	m_ctrlProgress.SetRange(0, 100);
}

void CDlgFTPTransfer::SetProgress(int nPercentage)
{
	int nLower, nUpper;
...
void CDlgFTPTransfer::SetTimeLeft(DWORD dwSecondsLeft, DWORD dwBytesRead, DWORD dwFileSize)
{
	CString sCopied;
	if (dwBytesRead < 1024)
	{
...
void CDlgFTPTransfer::SetStatus(const CString& sCaption)
{
	m_ctrlStatus.SetWindowText(sCaption);
}

void CDlgFTPTransfer::SetStatus(UINT nID)
{
	CString sCaption;
...
void CDlgFTPTransfer::SetStatus(UINT nID, const CString& lpsz1)
{
	CString sStatus;
...
void CDlgFTPTransfer::SetTransferRate(double KbPerSecond)
{
	CString sRate;
	if (KbPerSecond < 1)
	{
...
void CDlgFTPTransfer::PlayAnimation()
{
	m_ctrlAnimate.Play(0, (UINT)-1, (UINT)-1);
}

void CDlgFTPTransfer::HandleThreadErrorWithLastError(UINT nIDError, DWORD dwLastError)
{
...
void CDlgFTPTransfer::HandleThreadError(UINT nIDError)
{
...
void CDlgFTPTransfer::UpdateControlsDuringTransfer(DWORD dwStartTicks, DWORD& dwCurrentTicks, DWORD dwTotalBytesRead, DWORD& dwLastTotalBytes, 
...
void CALLBACK CDlgFTPTransfer::_OnStatusCallBack(HINTERNET hInternet, DWORD dwContext, DWORD dwInternetStatus, 
												 LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
{
	//Convert from the SDK C world to the C++ world
	CDlgFTPTransfer* pDlg = (CDlgFTPTransfer*) dwContext;
	ASSERT(pDlg);
	ASSERT(pDlg->IsKindOf(RUNTIME_CLASS(CDlgFTPTransfer)));
	pDlg->OnStatusCallBack(hInternet, dwInternetStatus, lpvStatusInformation, dwStatusInformationLength);
}

void CDlgFTPTransfer::OnStatusCallBack(HINTERNET /*hInternet*/, DWORD dwInternetStatus, 
									   LPVOID lpvStatusInformation, DWORD /*dwStatusInformationLength*/)
{
...
void CDlgFTPTransfer::OnDestroy() 
{
...
BOOL CDlgFTPTransfer::PreCreateWindow(CREATESTRUCT& cs) 
{
...
/*
BOOL CDlgFTPTransfer::DisplayFtpDir(
						  HWND hDlg,
						  ...
						  int nListBoxId )
{
	...
	hFind = FtpFindFirstFile( hConnection, TEXT( "/Material/2456/*.*" ), &dirInfo, dwFindFlags, 0 );
	if ( hFind == NULL )
	{
		dwError = GetLastError( );
		if( dwError == ERROR_NO_MORE_FILES )
		{
			//StringCchCopy(szMsgBuffer, MAX_PATH+8,TEXT( "No files found at FTP location specified." ) );
			StrCpy(szMsgBuffer,TEXT("No files found at FTP location specified"));
			retVal = TRUE;
	...
	do
	{
		...
		if( FAILED( StrCpy(szFName,dirInfo.cFileName) ) ||
			( ( dirInfo.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) &&
			( FAILED( StrCat(szFName,TEXT( " <DIR> ") ) ) ) ) )
		{
			StrCpy( szMsgBuffer,TEXT( "Failed to copy a file or directory name." ) );
			retVal = FALSE;
			goto DisplayDirError_2;
		}
		::SendDlgItemMessage( hDlg, nListBoxId, LB_ADDSTRING, 0, (LPARAM) szFName );
