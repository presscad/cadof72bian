...
{  
	...
	{
		eId=IdArray.at(i);
		acdbOpenObject(pE, eId, AcDb::kForWrite); 
		pE->erase();
		pE->close();                
	}
				                 ...
				                 int nHorzAlignType,int nVertAlignType,double dHeight,double dWidFactor,double dAngle, AcDbDatabase* pDB)
{
	...
	//创建文本
	AcDbText *pText = new AcDbText(ptAlign,strText,AcDbObjectId::kNull/*IdTextStyle*/,dHeight,dAngle);
 ...
 long CDistEntityTool::CreateTextStyle(AcDbObjectId& resultId,const char* csTsName,const char* szFlNm,const char* szBigFlNm,AcDbDatabase* pDB)
{
	...
	if(Acad::eOk != pTsTbl->getAt(csTsName,resultId)){
		...
		pTstRcd->setFileName("txt.shx");
		pTstRcd->setBigFontFileName("gbcbig.shx");
		pTstRcd->setTextSize(0.0);
...
long  CDistEntityTool::GetPolylinePts(AcDbPolyline *pPolyline,AcGePoint3dArray& ptArray)
{
	...
	nFound = -1;
	int nStart = ptArray.logicalLength()-1;
	if(Adesk::kTrue != ptArray.find(AcGePoint3d(VertPtArray[i].x,VertPtArray[i].y,0),nFound,nStart))
	{
		ptArray.append(AcGePoint3d(VertPtArray[i].x,VertPtArray[i].y,0));
	}
	if(pPolyline->isClosed())
...
AcDbObjectId CDistEntityTool::CreateBlockByDwgFile(CString strBlkName,CString strDwgFileName)//含路径的文件名(块名为文件名，不含路径)
{
	...
	{
		strDwgFileName.Format("%sBLOCKS\\BlockName.dwg",GetCurArxAppPath("shDistSdeData.arx"));	
	}
