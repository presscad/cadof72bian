...
public:
	...
	//返回：无
	virtual void GetSearchShapePointArray(IDistConnection* pConnection, const char* strSQL,AcGePoint3dArray* ptArray);

	virtual int CopyRecordFromOneToAnother(IDistConnection* pConnection, LPCTSTR lpSrcTable, LPCTSTR lpDestTable, LPCTSTR lpWhere);

	virtual void GetSearchFieldArray(IDistConnection* pConnection, LPCTSTR strSQL, LPCTSTR lpTable, LPCTSTR lpField, DIST_STRUCT_SELECTWAY* pFilter, CStringArray& array);
	...
	virtual long ReadCadLayerInfo(IDistConnection * pConnection, LPCTSTR lpCadLayer, int& nColorIndex, CString& lpLineType, AcDb::LineWeight& dLineWeigth);
	...
	virtual long ReadSdeDBToCAD(void* pObjIdArray,
		                        ...
		                        const char* strUserWhere=NULL,BOOL bSingleToSingle = TRUE,BOOL bIsReadOnly=FALSE, BOOL bDeleteExistEntity=TRUE);
	...
	virtual long SaveEntityToSdeDB(AcDbObjectIdArray& objIDs, BOOL bAfterSaveToDelete=TRUE, BOOL bSingleToSingle = TRUE);

	virtual long SaveEntityToSdeDB(AcDbObjectIdArray& objIDs, CString strXMID, BOOL bAfterSaveToDelete=TRUE, BOOL bSingleToSingle = TRUE);

	// 保存新增对象
	virtual long SaveInsertEntityToSdeDB(AcDbObjectIdArray& objIds, BOOL bAfterSaveToDelete=TRUE, BOOL bSingleToSingle = TRUE);

	// 保存修改对象
	virtual long SaveModifyEntityToSdeDB(AcDbObjectIdArray& objIds, BOOL bAfterSaveToDelete=TRUE, BOOL bSingleToSingle = TRUE);
...
public:
	...
	virtual long WriteToSDE(int* pRowId,IDistCommand* pCommand,AcDbObjectId& ObjId, long nType); // nType: insert=1; modify=2
