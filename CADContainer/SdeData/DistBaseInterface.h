...
{
	//1.连接数据库，并开始事务处理
	DIST_DATABASE_API int WINAPI gBeginTrans(char* strConnectString);

	//2.事务回滚
	DIST_DATABASE_API int WINAPI gRollbackTrans();

	//3.事务提交
	DIST_DATABASE_API int WINAPI gCommitTrans();

	//4.将指定条件的记录从一张表复制到另一张表
	DIST_DATABASE_API int WINAPI gCopyRecordFromOneToOther(const char* strSdeLyNameSrc, //源表
		const char* strSdeLyNameTrg, //目标表
		const char* strWhere, //查询条件
		bool bAddVersion = false, // 是否添加版本信息
		const char* strUserName = NULL, // 修改人
		const char* strCurTime = NULL); // 修改时间（2008-01-16）

	//5.删除指定条件的记录
	DIST_DATABASE_API int WINAPI gDeleteRecord(char* strSdeName, //图层名
		char* strWhere);  //图层名

	//6.更新图层中指定的记录信息
	DIST_DATABASE_API int WINAPI gUpdateCADInfo(char* strSdeName,     //图层名
		char* strWhere,       //查询条件
		char* strFldNames,    //字段名称格式为：fld1,fld2,...,fldn,
		char* strFldValues);  //字段值格式：v1,v2,...,vn,

	/*
	DIST_DATABASE_API int WINAPI gWriteTreeToXML(char* strFileXml,
	char* strServerName,
	char* strInstance,
	char* strDataBaseName,
	char* strUserName,
	char* strPassWorld);
	*/
}
...
struct IDistCommand
{
	...
	// 获取服务器名称
	virtual long GetServerTime(struct tm& tm_server) = 0;
	...
	//       nPtCount    坐标点个数
	// 返回：成功返回 1，失败返回 0，其他参考定义
	virtual long CreateShapeByCoord(int* pShape,const char* strLyName,IDistPoint* ptArray,int nPtCount) = 0;
	...
	virtual long CopyRecordByCondition(const char* strTargeLyName,const char* strSrcLyName,const char* strWhere, 
		bool bAddVersion = false, const char* strUserName = NULL, const char* strCurTime = NULL) = 0;

	// 跨图层拷贝对象
	virtual long CopyRecordsAcrossLayer(LPCTSTR lpDestLayer, LPCTSTR lpSrcLayer, CUIntArray& rowIds, LPCTSTR lpUserName, int nEditRowId) = 0;
...
struct IDistRecordSet
{
	...
	virtual long GetValueAsString(long& nType, char *strValue,int nIndex)=0;
	...
	virtual long GetValueAsString(long& nType, char *strValue,const char* strFldName)=0;
