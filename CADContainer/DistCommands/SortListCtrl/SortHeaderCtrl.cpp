...
void CSortHeaderCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	int iColumnCount = GetItemCount();
	int nColumn = -1;
	...
	if(nColumn >=0 && nColumn < iColumnCount)
	{
		CPoint nowPoint;
		::GetCursorPos(&nowPoint);

		GetParent()->GetParent()->SendMessage(WM_ON_POPMENU,WPARAM(&nowPoint),nColumn);
	}
	
//	CSortHeaderCtrl::OnRButtonDown(nFlags, point);
}
