...
BOOL CDistReduceEntity::ReduceEntities(AcGePoint3dArray SelectPtArray,const char* strLyName/*=NULL*/)
{
	...
	CString strLB, strRT;
	strLB.Format("%f,%f", dminX, dminY);
	strRT.Format("%f,%f", dmaxX, dmaxY);
...
/*		BOOL bConvert = FALSE;
		...
		if(pEnt->isKindOf(AcDb2dPolyline::desc()))
		{
			...
			pTempline = new AcDbPolyline;
			bConvert = TRUE;

			pTempline->convertFrom(pEnt, Adesk::kFalse);
			pCurve = (AcDbCurve*) pTempline;
		}
*/		if (pEnt->isKindOf(AcDb3dPolyline::desc()))
		{
			AcDbSpline* spline = NULL;   //这两种类型实体需要转换，才能 getSplitCurves 成功
			Acad::ErrorStatus es = pEnt->handOverTo(spline);
